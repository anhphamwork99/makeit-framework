---
name: tl-progress
description: TL sprint progress — display current phase, deliverable status, and next actions
---

<purpose>
Display comprehensive TL sprint progress showing lifecycle stage, deliverable completion status, gate results, and immediate next actions.
</purpose>
...
<process>
  ...
  <step name="display_progress">
    ...
    Deliverables:
    ├── Task breakdowns: {N}/{total} created
    ├── API contracts: {N}/{total} defined
    ├── Estimations: {N}/{total} done
    └── Code reviews: {N}/{total} completed

    Gate Status:
    ├── Gate 2 (input): ✅ passed
    └── Gate 3 (output): ⏳ pending
