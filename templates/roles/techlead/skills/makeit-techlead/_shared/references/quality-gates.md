...
> ⭐ **TECHLEAD VERIFIES THIS GATE — Bạn là RECEIVER, verify BA stories trước khi break tasks**
> Dùng `/makeit:clarify` (Gate 2 check is built into stage-clarify).

## Gate 2: BA → Techlead

**Verifier:** Techlead | **Trigger:** Stories ready

| # | Check Item | TL Action |
|---|-----------|-----------|
| 1 | User story follows format: "As a [role], I want [action], so that [benefit]" | ⭐ **VERIFY** |
| 2 | Acceptance criteria listed (≥ 3 checkable items) | ⭐ **VERIFY** |
| 3 | Figma design link attached and accessible | ⭐ **VERIFY** |
| 4 | Edge cases documented (empty state, error state, boundary conditions) | ⭐ **VERIFY** |
| 5 | Dependencies identified and status confirmed | ⭐ **VERIFY** |
| 6 | Source context rõ ràng: reference đến PO Goal + Design specs | ⭐ **VERIFY** |
| 7 | Lark task has correct sprint, priority, and assignee | ⭐ **VERIFY** |

**Fail →** Techlead tag BA trên Telegram, ghi rõ story thiếu.

---

> ⭐ **TECHLEAD OUTPUT GATE — Bạn là SENDER, verify output trước khi handoff cho Dev**
> Dùng `/makeit:check-gate` hoặc `/makeit:self-review` để self-verify.
...
| # | Check Item | TL Action |
|---|-----------|-----------|
| 1 | Task break rõ ràng: FE tasks tách biệt BE tasks | ⭐ **YOUR OUTPUT** |
| 2 | Scope cụ thể: mô tả rõ cần implement cái gì | ⭐ **YOUR OUTPUT** |
| 3 | User story reference: link đến user story của BA | ⭐ **YOUR OUTPUT** |
| 4 | Technical constraints noted (API limits, browser support, third-party) | ⭐ **YOUR OUTPUT** |
| 5 | API contract defined (nếu FE/BE cần coordinate) | ⭐ **YOUR OUTPUT** |
| 6 | Estimation hợp lý: Techlead đã estimate effort | ⭐ **YOUR OUTPUT** |
| 7 | Lark task has correct sprint, priority, and assignee | ⭐ **YOUR OUTPUT** |

**Fail →** Dev tag Techlead trên Telegram, ghi rõ task thiếu.

---

> ⭐ **TECHLEAD PARTICIPATES — Review code quality, architecture alignment, AI-generated code**
...
> ⭐ **TECHLEAD PARTICIPATES — Ensure architecture decisions upheld, coding standards enforced**
