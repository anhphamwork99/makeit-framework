...
<objective>
Present the Agent's proposed approach for implementing a phase — surfaces assumptions, trade-offs, and intended direction BEFORE plan creation. Lets user approve, adjust, or redirect before effort is invested in detailed planning.
</objective>

<execution_context>
@.agent/skills/makeit-ba/ba-planning/stage-show-phase-approach.md
</execution_context>
...
<process>
  ...
  <step name="analyze_phase">
    Read `.makeit/sprint/SPRINT-*/ROADMAP.md` for phase goal.
    Read CONTEXT.md if exists (locked decisions from discuss-phase).
    Read SPECS.md for sprint scope awareness.
    Analyze what the phase requires and formulate an approach.
  </step>

  <step name="present_approach">
    Display structured approach proposal:
    - **Goal interpretation** — how Agent understands the phase goal
    - **Proposed approach** — high-level steps Agent would take
    - **Key assumptions** — what Agent is assuming (for user to validate)
    - **Trade-offs considered** — alternatives Agent considered and why this approach
