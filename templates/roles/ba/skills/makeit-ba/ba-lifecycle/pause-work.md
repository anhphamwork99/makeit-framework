---
name: ba-pause-work
description: BA context save — save sprint context and create handoff state for later resume
---

<purpose>
Save current BA sprint context so work can be resumed later in a fresh session without losing progress.
</purpose>
...
**Khi có thể có session khác đang modify cùng workspace files:** Trước khi pause, check git status cho uncommitted changes từ session khác. Nếu phát hiện → ghi cảnh báo trong pause context: "⚠️ Other session may have modified files. Run `git diff` khi resume để reconcile." Không overwrite changes — preserve both versions.
