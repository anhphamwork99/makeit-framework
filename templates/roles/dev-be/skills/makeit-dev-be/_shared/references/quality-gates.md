...
> ⭐ **DEV BE VERIFIES THIS GATE — Bạn là VERIFIER tại gate này**
> Check task từ Techlead trước khi implement. Gate 3 check is built into `/makeit:clarify`.

## Gate 3: Techlead → FE/BE

**Verifier:** Dev FE/BE | **Trigger:** Tasks assigned

| # | Check Item | Dev BE Focus |
|---|-----------|-------------|
| 1 | Task break rõ ràng: FE tasks tách biệt BE tasks | ⭐ Verify BE tasks rõ ràng |
| 2 | Scope cụ thể: mô tả rõ cần implement cái gì | ⭐ API endpoints + business logic rõ |
| 3 | User story reference: link đến user story của BA | ⭐ Cross-check với BA story |
| 4 | Technical constraints noted (API limits, third-party) | ⭐ BE-specific constraints |
| 5 | API contract defined (endpoint, request/response) | ⭐ Critical cho BE |
| 6 | Estimation hợp lý: Techlead đã estimate effort | ⭐ Check realistic |
| 7 | Lark task has correct sprint, priority, and assignee | ⭐ Assigned đúng |

**Fail →** Dev tag Techlead trên Telegram, ghi rõ task thiếu.

---

> ⭐ **DEV BE OUTPUT GATE — PR của bạn phải pass gate này**
> Self-review trước khi submit. Dùng `/makeit:self-review` + `/makeit:check-gate`.
...
| # | Check Item | Dev BE Focus |
|---|-----------|-------------|
| 1 | PR follows template (all required sections filled) | ⭐ Fill all sections |
| 2 | Lark task linked in PR description | ⭐ Link task |
| 3 | AI Review Checklist completed (all items checked) | ⭐ Complete checklist |
| 4 | Self-reviewed diff before requesting review | ⭐ `/makeit:self-review` |
| 5 | Commit messages follow conventional commits | ⭐ feat/fix/docs/chore |
| 6 | FE: Screenshot/video attached for UI changes | N/A (FE only) |
| 7 | BE: API documentation updated for endpoint changes | ⭐ **YOUR ITEM** |
| 8 | No console errors/warnings in development | ⭐ Check logs |
| 9 | Code follows coding standards | ⭐ Follow standards |
