---
name: fe-pause-work
description: FE context save — save sprint context and create handoff state for later resume
---

<purpose>
Save current FE sprint context so work can be resumed later in a fresh session without losing progress.
</purpose>
...
<process>
  <step name="capture_state">
    ...
    3. **In-progress files** → any partially created components
  ...
  <step name="create_pause_context">
    ...
    - **Task:** {what was being done — which component, which state}
    - **Status:** {where in task — extracting, implementing, verifying}
    - **In-progress files:** {list of partially complete components}
