...
| 3 | Techlead | User stories từ BA | FE tasks + BE tasks + API contracts | Gate 3: ⭐ **Dev FE verifies** |
...
PO Goal & Context ──────── WHY we build (business value)
    │
    ▼
Designer Specs ─────────── HOW it looks (visual design) ← VISUAL SOURCE OF TRUTH
    │
    ▼
...
| Code không match Figma | **Figma wins** → fix code hoặc clarify với Designer |
